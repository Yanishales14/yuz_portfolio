  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      <div className="absolute inset-0 z-0">
        ...
        <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse at center, transparent 40%, rgba(250,250,249,0.5) 100%)' }} />
      </div>

      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        <motion.div className="absolute top-1/4 left-[10%] w-24 h-24 rounded-2xl bg-white/[0.03] border border-white/[0.06]" animate={{ y: [-10, 10, -10], rotate: [0, 2, -2, 0] }} transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut' }} />
        <motion.div className="absolute top-1/3 right-[12%] w-16 h-16 rounded-full bg-white/[0.03] border border-white/[0.06]" animate={{ y: [10, -10, 10] }} transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut', delay: 1 }} />
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 text-center pt-32 pb-32">
        ...
        <motion.h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-[5.5rem] xl:text-[6.5rem] font-bold tracking-[-0.02em] leading-[1.05] mb-8" style={{ fontFamily: "'Space Grotesk', sans-serif" }} initial={{ opacity: 0, y: 50 }} animate={animate ? { opacity: 1, y: 0 } : {}} transition={{ duration: 1.2, delay: 0.2, ease: [0.16, 1, 0.3, 1] }}>
          Crafting Visual<br /><span className="text-muted-foreground/50">Stories That</span><br />Captivate
        </motion.h1>
        ...
        <motion.div className="flex flex-wrap items-center justify-center gap-4" initial={{ opacity: 0, y: 20 }} animate={animate ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, delay: 0.7, ease: [0.16, 1, 0.3, 1] }}>
          <motion.a href="#showreel" className="group flex items-center gap-3 px-8 py-4 bg-foreground text-background rounded-2xl font-medium shadow-lg hover:shadow-xl transition-shadow" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
            <div className="w-8 h-8 rounded-full bg-background/20 flex items-center justify-center"><Play size={14} className="ml-0.5" fill="currentColor" /></div>
            Watch Showreel
          </motion.a>
