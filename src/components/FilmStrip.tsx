...
export function FilmStrip() {
  ...
  // Infinite scroll: duplicate items enough times to fill viewport + seamless loop
  // We need enough copies that translateX(-50%) creates a perfect loop
  const duplicates = Math.max(8, Math.ceil(20 / Math.max(projects.length, 1)));
  const stripItems = [...Array(duplicates)].flatMap(() => projects);

  // Set CSS custom property for animation duration based on content width
  useEffect(() => {
    if (scrollerRef.current) {
      const totalWidth = scrollerRef.current.scrollWidth;
      // ~50px per second for smooth scroll
    ...
    <section className="py-0 overflow-hidden" ref={ref}>
      ...
      >
        <div className="bg-card border-y border-border relative">
          ...
          {/* Thumbnail strip — the main scrolling content */}
